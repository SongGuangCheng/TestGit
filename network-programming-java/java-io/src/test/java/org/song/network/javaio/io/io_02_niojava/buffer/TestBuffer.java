...
/**
 * NIO 中 buffer的使用 注意: buffer使用比较麻烦, netty框架对其进行了封装,
 */
public class TestBuffer {
    ...
    public static void main(String[] args) {
        ...
        buf.put(13);// position位置：0 - > 1 put方法会改变position位置
        buf.put(21);// position位置：1 - > 2
        buf.put(35);// position位置：2 - > 3

        //把位置复位为0，也就是position位置：3 - > 0
        // put和get之后 要复位 TODO 这一块要好好理解
        buf.flip();
        ...
        for (int i = 0; i < buf.limit(); i++) {
            //调用get方法会使其缓冲区位置（position）向后递增一位 get方法会改变position位置 TODO 这一块要好好理解
...
//
//		IntBuffer buf2 = IntBuffer.wrap(arr, 0 , 2); // 入参一个数组, 后面两个参数是截取放入数组的指定范围,// wrap方法 limit就是数组长度
